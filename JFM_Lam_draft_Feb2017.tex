...
\def~{\hphantom{0}}
  \begin{tabular}{lccccccc}
      $Case$      		& ~$I^*$		& ~$Re$		& ~$Fr$ 		& ~~$Ri$	& $N$ (rad/sec)		& $U$ (cm/sec)		& $A$ (mm/sec) 	\\[3pt]
...
\begin{figure}
 ...
 ssed in probability per bin, for 500 free-fall fluttering discs in (a) water and (b) stratified flow ($Fr = 0.9$), initially released from ($x,y$) = (0,0). (c) Histogram of the range distribution. (d) The cumulative distribution function $(cdf)$. $x$, $y$, and $r$ are normalized by the fluid depth, $h$. }
\label{fig:landing}
\end{figure}
...
The analysis of the landing sites of descending discs provide insight on how stratification influence the radial dispersion. However, it provides no temporal and state information of the descent motion. So, to investigate further, trajectories are reconstructed in water and stratified flow, at $Fr = 1.51$ ($N = 1.06$ rad/sec) and $Fr = 0.90$ ($N = 1.69$ rad/sec), with focus on the latter. The discs are released with near zero initial conditions top center of the tank, with the $x$-axis along the front of the tank (left-to-right), $y$-axis along the depth of the tank, and $z$-axis along the height of the tank centered at the initial drop location, as illustrated in figure \ref{fig:introimage}. Coin descents are recorded and processed as described in \S\ref{sec:method} for reconstructing the position and orientation information.
...
\\where $\rho_0$ is the reference density of the fluid, $\rho$ is the density of the disc, $N$ is the Brunt-V{\"a}is{\"a}l{\"a} freqency, and $\theta$ is the inclination or nutation angle. The equation is derived by finding the torque on the disc from the offset between the gravitational and buoyancy forces. See \S{\ref{appA}} for the derivation. As an example, for $N = 1.69~rad/sec$, equation \ref{eq:restoretorque} gives a maximum radial restoring acceleration of $2.49~rad/sec^2$ at $\theta = \pm \pi/4$, and zero at $\theta = 0$ (when the disc is parallel to the horizontal plane) and $\theta = \pm \pi/2$ (when the disc is perpendicular to the horizontal plane).  Equation \ref{eq:restoretorque} provides a non-linear relation with $\theta$, that is similar to the motion of pendulum. 
...
Reconstructed results between descents in water and descents in stratified flows are summarized in figure \ref{fig:summary_recon} and in table \ref{tab:reconsummary}, including the results from the $Fr = 1.51$ ($N = 1.07~rad/sec$) stratified flow cases. %For the average radial range and range rate, little can be decipher 
...
\def~{\hphantom{0}}
  \begin{tabular}{lcccccc}
      ~~$Fr$~~		& $\left<\theta_{p}\right>/\left<\theta_{p}\right>_w$	& $\left<\dot{\theta}_{p}\right>/\left<\dot{\theta}_{p}\right>_w$	& $\left<\dot{z}\right>/\left<\dot{z}\right>_w$	 & $\left<\ddot{z}\right>/\left<\ddot{z}\right>_w$ \\[3pt]
