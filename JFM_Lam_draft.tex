...
As an example, figure \ref{fig:waterCase8vsstratCase10} shows two reconstructed fluttering descents released from rest, one in pure water and another in $\textrm{Fr} = 0.90$ stratified fluid. Snap shots views at different time intervals are shown for both cases \ref{fig:waterCase8vsstratCase10}, where the viewing angle have been maximized to show as much of the fluttering motion as possible. The time interval was selected to show approximately 1 cycle of the fluttering motion. During the descent the disc can gyrate and the fluttering motion can no longer be observed in certain angles, as shown in the water example. From figure \ref{fig:waterCase8vsstratCase10}, we note two clear observations regarding the two cases: (1) for one cycle, the water descents duration are about 1.3 seconds for both the cycle near the top and near the bottom, and descending about 3.2 $D$ (disc's diameter) each cycle, or roughly at 2.5 $D$/sec. (2) For the stratified flow, we note that near the begining of the descent that descent time for one cycle is approximately 1.4 seconds covering about 2.5 $D$, and the near the bottom the stratified descent is covering about 2.5 $D$ in 1.6 seconds, or roughly a decrease in speed from 1.8 $D$/sec to 1.6 $D$/sec between the two cycles. This observation is consistent with intuition, since the density of the stratified fluid is increasing with depth we expect the drag to increase proportionally, thus, decelerating the descending disc and increase the descent time. 
...
The decrease in the fluttering amplitude in stratified flow can be observed in figure \ref{fig:amplitude}. 
...
The $C_D$ for stratified flow is larger than that of water, $C_{D}^{s}/C_{D}^{w} = 1.37$.  This increase or enhanced drag has also been observed by \citet{torres2000flow} and \citet{yick2009} for vertical motion of spheres in stratified flow, where they found correlations between $C_D$ and the Froude number $\textrm{Fr}$, and was investigated further by \citet{doostmohammadi2014}. This approximate 30$\%$ increase in the drag coefficient has a major implication on modeling and prediction of the trajectory in stratified flow, where the data suggest that there is a shift in the \textit{a priori} mean drag coefficient value. This enhance drag can also be interpreted as an increase in the timing error in a descent model, if the nominal drag coefficient is not corrected.  
